    ...
    $.extend({tablesorterColumnSelector: new function() {
        ...
        enableColumnSelector = function(table, c, triggered) {
            ...
            $('thead tr:first th', table).each(function() {
                ...
                columnSelectorPane += '<li><input type="checkbox" class="fast-checkbox" value="' + rowid + '" name="col' + rowid + '" id="col' + rowid + '" /><label for="col' + rowid + '" class="fast-label" id="label-col' + rowid + '">' + name + "</label></li>\n";
                if (c.layout == 'vertical') {
                    columnSelectorPane += '<br />' + "\n";
                }
            });
            $(c.container).html('<ul class="forms-inline-list columns-list"><li>Выберите колонки для отображения:&nbsp;&nbsp;</li>'+columnSelectorPane+'</ul>').trigger('updateState');
