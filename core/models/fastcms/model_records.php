...
//							print_r($item->$column);
  							...
  							if (isset($tipo['fields'][$column]['type']))
  							{
  								...
  								else if ($tipo['fields'][$column]['type'] == 'repeatable')
  								{
  ...
  public function uri_is_used($uri='')
  {
		...
		$fromwhere = implode(",",$this->get_types());
		
    if ($uri != '')
    {
    	$result = $this->type($fromwhere)->where('uri', $uri)->get();
  ...
  {
		$types = array();
  		$results = $this->db->select('name')->from('types')->get()->result_array();
  ...
  public function get_linear_types()
  {
		$types = array();
  		$res = $this->db->select('title, id_type')->from('types')->where('tree','n')->get();
  		$results = $res->result_array();					 
	  			foreach ($results as $result)
	  			{
	  				$types[$result['id_type']] = $result['title'];
	  			}
