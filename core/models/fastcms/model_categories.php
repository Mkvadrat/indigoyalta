...
Class Model_categories extends CI_Model
{
	...
	* @return array
	*/
	public function get_categories_names($ids_category,$typo)
	{
		...
		{
			$this->db->cache_on();
	  		$categories = $this->db->select('category_name')
	  						  	   ->from('categories')
	  						   	   ->where_in('id_category', $ids_category)
	//  						   	   ->where('id_type', $typo)
	  						       ->get()->result_array();
	  		...
	  		if (count($categories))
	  			...
	  			{
	  				$record_categories[] = $category['category_name'];
	  			}
		}
