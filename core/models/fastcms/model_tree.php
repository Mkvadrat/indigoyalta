...
Class Model_tree extends CI_Model
{
	...
	public function __construct()
	{
		...
		{
			show_error('The default content type has not been defined in the file config/fastcms.php!');
		}
    ...
    public function category($category) {
        if (!in_array($category, $this->_categories)) return;
	...
	private function _fetch($stage = FALSE)
	{
        ...
        if (!$this->_category) {
            ...
            switch($this->_category) {
                case 'rent':
                    $this->db->where('show_in_menu_rent !=', 'F');
                    break;
                default:
                    $this->db->where('show_in_menu !=', 'F');
                    break;
	...
	*/
	 ...
	 * @param int|string $type
	 */
	public function get_type_cachepath($type = '', $suffix = '')
	{
	...
	public function get_type_cachepaths($type = '')
	{
        ...
        foreach ($this->_categories as $category) {
            ...
            foreach ($languages as $lang)
            {
                $tmp[] = str_replace('{name}', $lang.'-'.$_name, $source_name);
                $tmp[] = str_replace('{name}', $this->stage_prefix.$lang.'-'.$_name, $source_name);
            }
        }
    ...
    public function get_by_category($category, $type='')
    {
        $this->category($category);
        $tree_file = $this->get_type_cachepath($type='', $category);
        return $this->_cache_tree($tree_file);
    }
	 ...
	 * This method is useful to make a fake URI and let Fastcms interpret it
