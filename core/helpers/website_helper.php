 ...
 * to improve performances instead of using the fastcms()
 * function declared below.
 */
$GLOBALS['B'] = CI_Controller::get_instance();

/**
 * Returns the Fastcms singleton super-object
...
function minify($files = array(), $version='') 
{
	...
	$folder = config_item('attach_out_folder') . 'min/resources-'.$ext.'/' . md5($current_theme.implode(',', $files));
...
function semantic_category_url($object = '', $cats)
{
	global $B;
	$view =& $B->view;
	$record = $view->get('record');
...
//	print_r($object);
	$cat_names = $B->categories->get_categories_names($cats,$object->get('id_type'));
//	print_r($cat_names);
	
	//If we are here, is a normal page. We have to try to find another page that is listing this record
	if ($page && $object instanceof Record)
	{
		$linkdata = $B->pages->get_category_semantic_url($object->get('id_type'), $cat_names);
		
		return site_url($linkdata . '/' . $object->get('uri'));
	} else {
		//No more things to try on
		return '#';
	}
