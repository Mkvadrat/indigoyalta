...
Class Core_Contents extends Fastcms_Controller
{
    ...
    public function record_list($tipo='', $page=0, $per_page = 0)
    {
       ...
       $per_page = $per_page != 0 ? $per_page : $this->config->item('records_per_page');
        ...
        $pagination = array(
            	...
            	'num_links'		=> 1,
            	'first_tag_open'		=> '<li class="btn">',
            	'first_tag_close'		=> '</li>',
            	'last_tag_open'		=> '<li class="btn">',
            	'last_tag_close'		=> '</li>',
            	'full_tag_open'		=> '<ul class="btn-group pager-links text-right">',
            	'full_tag_close'		=> '</ul>',
            	'next_tag_open'		=> '<li class="btn">',
            	'next_tag_close'		=> '</li>',
            	'prev_tag_open'		=> '<li class="btn">',
            	'prev_tag_close'		=> '</li>',
            	'num_tag_open'		=> '<li class="btn">',
            	'num_tag_close'		=> '</li>',
            	'cur_tag_open'		=> '<li class="btn btn-blue"><span>',
        ...
        //We get the records
		//var_dump($type['fields']["date_publish"]);
...
//				echo '<pre>'.print_r($records).'</pre>';
		...
		//echo $tipo['id'];
		$recordcats = $this->categories->type($type['id'])->order_by('name', 'ASC')->get();
		$filtercats = array();
		foreach($recordcats as $recordcat){
			}
//		print_r($recordcats);
 		$this->view->set('recordcats', $recordcats);
    ...
    * @param int $record_id
    */
    public function add_record_images($type='', $record_id='', $child_id='')
    {
    ...
    public function type_categories($type = '') {
		...
		if (strlen($category_name)) {
			if (!$this->categories->exists($tipo['id'], $category_name)) {
  			...
  			if ($done) {
  				$this->view->set('success', $this->lang->_trans('The category %n has been added.', array('n' => '['.$category_name.']')));
  			} else {
  				$this->view->set('error', $this->lang->_trans('Cannot insert the category %n.', array('n' => '['.$category_name.']')));
  			}
			} else {
				$this->view->set('alert', $this->lang->_trans('A category named %n already exists.', array('n' => '['.$category_name.']')));
			}
		}
    ...
    */
    ...
    {
		$types = array();
  		$results = $this->db->select('title, id_type')->from('types')->where('tree','n')->get()->result_array();
  							 
	  			foreach ($results as $result)
	  			{
	  				$types['id_type'] = $result['title'];
	  			}
