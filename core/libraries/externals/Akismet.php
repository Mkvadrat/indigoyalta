 ...
 * 
 * This class has been adapted to Fastcms. http://getfastcms.com
 * 
...
class Akismet extends Core
{
	...
	public function isKeyValid() {
		...
		$response = $this->sendRequest('key=' . $this->fastcmsApiKey . '&blog=' . $this->websiteURL, $this->akismetServer, '/' . $this->akismetVersion . '/verify-key');
	...
	private function sendRequest($request, $host, $path) {
		...
		$http_request .= "User-Agent: Fastcms " . FASTCMS_VERSION . " | Akismet/1.11\r\n";
		$http_request .= "\r\n";
	...
	{
		$response = $this->sendRequest($this->getQueryString(), $this->fastcmsApiKey . '.rest.akismet.com', '/' . $this->akismetVersion . '/comment-check');
		
		if($response[1] == 'invalid' && !$this->isKeyValid()) {
			log_message('error', 'The Fastcms API key passed to the Akismet constructor is invalid.  Please obtain a valid one from http://akismet.com/get');
			return FALSE;
		}
