...
Class Content
{
	...
	public function add_type($options)
	{
		...
		if (write_file($storage_path, $scheme)) {
			...
			$this->CI->load->users();
			$acl_id = $this->CI->users->add_acl('content', $type_name, 'Управление ' . $type_name);
	...
	public function type_name($type_id)
	{
		if (isset($this->content_types[$type_id]))
		{
			...
			show_error('['.$type_id.'] не найден. (content/type_name)', 500, 'Ошибка при получении данных с БД');
		}
	}
	...
	/**
	 * Register a new package to be used inside the core of Fastcms
	 * @param string $package
	 */
	public function register_package($package = '')
	{
		$this->CI->load->settings();
		if ($package != '' && $package instanceof Fastcms_Package) {
