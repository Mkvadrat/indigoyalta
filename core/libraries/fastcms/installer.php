...
Class Installer
{
	...
	public function create_groups()
	{
		...
		$acls = array();
		$acls[]= $this->users->add_acl('users', 'list', 'Просмотр пользователей');
		$acls[]= $this->users->add_acl('users', 'add', 'Управление пользователями');
		$acls[]= $this->users->add_acl('users', 'groups', 'Управление группами доступа');
		$acls[]= $this->users->add_acl('types', 'add', 'Добавление разделов');
		$acls[]= $this->users->add_acl('types', 'manage', 'Управление схемами сайта');
		$acls[]= $this->users->add_acl('types', 'delete', 'Удаление схем сайта');
		$acls[]= $this->users->add_acl('settings', 'manage', 'Управление настройками сайта');
		$acls[]= $this->users->add_acl('hierarchies', 'manage', 'Управление связями');
...
//					'description'		=> $type,
					...
					'label_new'			=> ($type == 'Menu' ? 'Новая страница' : $type),
	...
	public function create_indexes()
	{
		...
		foreach ($indexes as $index)
		{
			...
			$sql = 'CREATE INDEX idx_fastcms_'.$index_name.' ON '.$this->CI->db->dbprefix.$table.' ('.$column.');';
	...
	public function create_directories()
	{
		$directories = array(
			...
			$this->CI->config->item('fr_cache_folder'),					//Fastcms Cache files,
	...
	public function populate_settings($theme = '')
	{
		...
		$this->CI->settings->set('website_name', 'Название сайта');
	...
	public function create_premade($type = '')
	{
		...
		switch (strtolower($type))
		{
			case 'full':
				...
				$post->set('title', _('Тестовая запись'))
					 ->set('content', _('Алоха!!!'))
