 ...
 *
 * This is the main class of every single record.
 *
 * @package		FastCMS
 * @author		Fastimus.ru
    		...
    		if (!$tipo) return;
	      	...
	      	if ($xmltree)
	      	{
		      	foreach ($xmltree as $field_name => $field_value)
		      	{
		      		...
		      		} else if ($field_type == 'repeatable')
		      		{
				//		log_message('error', '' . $this->_data[$field_name] . '');
//						echo '<pre>';
//						print_r(unserialize($this->_data[$field_name]));
//						echo '</pre>';
		      			$this->_data[$field_name] = unserialize($this->_data[$field_name]);
//						$this->_data[$field_name] = $this->_data[$field_name];
//						print_r($this->_data[$field_name]);
