...
Class Xml
{
      		...
      		foreach ($tipo['fields'] as $field_name => $field)
      		{
        		...
        		if (!in_array($field_name, $tipo['columns']))
        		{
          			...
          			if ($value != '') {
            			...
            			if (is_array($value))
            			{
							if($field_name == 'scheduleiwf'){
								$value = serialize($value);
							//	$value = '|'.$this->r_implode('|', $value ).'|';
							}else{
              					$value = '|'.implode('||', $value).'|';
							}
            			}
	...
	//-----------------------------------------------------------
	function data2XML(array $data, SimpleXMLElement $xml)
    {
        ...
        foreach($data as $key => $val) {
            if(is_array($val)) {
                ...
                $node->addChild($nodes, $this->data2Xml($val, $node));
            } else {
      ...
      foreach ($yaml['fieldsets'] as $fieldset) {
        ...
        if (isset($fieldset['fields']) && is_array($fieldset['fields'])) {
            ...
            foreach ($fieldset['fields'] as $field_name => $field) {
                ...
                //String options
                //Nodes
                foreach (array('kind', 'type', 'description', 'length', 'default', 'rules', 'resized', 'thumbnail','note','tooltip','additionaldesc','appenddata','prependdata',
    ...
    function parse_scheme($filepath)
    {
      ...
      switch ($ext) {
    	...
    	$field_usable_inputs = array(
      		'text','colorpicker', 'textarea', 'textarea_short', 'adress', 'date', 'checkbox', 'select', 'multiselect', 'radio', 'password','infoblock','separator','repeatable',
      		'images','gridimages', 'files', 'number', 'textarea_full', 'textarea_code', 'datetime', 'hidden', 'hierarchy', 'menuitemseditor', 'visualbuilder', 'attachments'
    	);
    	...
    	foreach ($node->fieldset as $fieldset_node)
    	{
            ...
            if (isset($fieldset_attr->icon))
            {
      		...
      		foreach ($fieldset_node->field as $field)
      		{
        		...
        		$content_field = array(
          			...
          			'additionaldesc'		=> isset($field->additionaldesc) ? (string)$field->additionaldesc : '',
          			'appenddata'		=> isset($field->appenddata) ? (string)$field->appenddata : '',
          			'prependdata'		=> isset($field->prependdata) ? (string)$field->prependdata : '',
            ...
            if ($field_name == 'id_type') {
              $content_field['default'] = (int) $content['id'];
        		...
        		if ($content_field['type'] == 'files' || $content_field['type'] == 'images' || $content_field['type'] == 'gridimages')
        		{
        		...
        		if ($custom_kind)
        		{
      			...
      			if ($content_field['type'] == 'images' || $content_field['type'] == 'gridimages')
        		{
        		...
        		if ($content_field['type'] == 'gridimages' || $content_field['type'] == 'images' || $content_field['type'] == 'files')
        		{
