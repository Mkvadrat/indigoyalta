...
class CI_Pagination {
	...
	protected $url_params = array(); // Additional URL params to all the links
	...
	public function create_links() {
		...
		# Если записей нет или не установлено количетсво записей на странице, то пейджер не нужен
		...
		# Если всё уместилось на одной странице, то пейджер не нужен
		if ($num_pages == 1)
			return '';
		...
		if ($CI->config->item('enable_query_strings') === TRUE OR $this->page_query_string === TRUE) {
			...
			$this->base_url .= parse_url($this->base_url, PHP_URL_QUERY) == '' ? '' : '&amp;';
		...
		if ($this->first_link !== FALSE AND $this->cur_page > ($this->num_links + 1)) {
			...
			$output .= $this->create_link($first_url, $this->first_link);
		}
		...
		if  ($this->prev_link !== FALSE AND $this->cur_page != 1) {
			...
			if (($i == 0 OR ($this->use_page_numbers && $i == 1)) AND $this->first_url != '')
				$output .= $this->create_link ($this->first_url, $this->prev_link);
			else {
				$i = ($i == 0) ? '' : $this->prefix.$i.$this->suffix;
				$output .= $this->create_link($this->base_url.$i, $this->prev_link);
			}
			...
			for ($loop = $start -1; $loop <= $end; $loop++) {
				...
				if ($i >= $base_page) {
					if ($this->cur_page == $loop)
						...
						if ($n == '' && $this->first_url != '')
							$output .= $this->create_link($this->first_url, $loop);
						else {
							$n = ($n == '') ? '' : $this->prefix.$n.$this->suffix;
							$output .= $this->create_link($this->base_url.$n, $loop);
						}
					}
				}
			}
		...
		if ($this->next_link !== FALSE AND $this->cur_page < $num_pages)
		{
			...
			$output .= $this->create_link($this->base_url.$this->prefix.$i.$this->suffix, $this->next_link);
		}
		...
		if ($this->last_link !== FALSE AND ($this->cur_page + $this->num_links) < $num_pages)
		{
			...
			$output .= $this->create_link($this->base_url.$this->prefix.$i.$this->suffix, $this->last_link);
		}
	...
	private function create_link($url, $title) {
		...
		return $this->num_tag_open.'<a '.$this->anchor_class.'href="'.$url.'">'.$title.'</a>'.$this->num_tag_close;
	}
}
