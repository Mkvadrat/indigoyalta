...
Class Fastcms_Loader extends CI_Loader {
	...
	/**
	 * This function loads the Fastcms environment
	 */
	function fastcms()
	{
		...
		//Standard Fastcms Libraries
		$this->library(
			array(
				FASTCMS . '/content', FASTCMS . '/xml', FASTCMS . '/view'
			)
		);

		//Class that create instances of records
		require_once(FASTCMSLIB . 'record.php');
	...
	/**
	 * Loads a model inside Fastcms framework
	 * @param string $model
	 * @param string $name
	 * @param bool $db_conn
	 */
	...
	{
		$this->model(FASTCMS.'/'.$model, $name, $db_conn);
	}

	/**
	 * Loads a library inside Fastcms framework
	 * @param string $library
	 * @param string $name
	 */
	function frlibrary($library, $name = NULL)
	{
		$this->library(FASTCMS.'/'.$library, NULL, $name);
	}
	...
	function records()
	{
		$this->model(FASTCMS.'/model_records', 'records');
	}
	...
	function tree()
	{
		$this->model(FASTCMS.'/model_tree', 'tree');
	}
	...
	function documents()
	{
		$this->model(FASTCMS.'/model_documents', 'documents');
	}
	...
	function categories()
	{
		$this->model(FASTCMS.'/model_categories', 'categories');
	}
	...
	function auth()
	{
		$this->model(FASTCMS.'/model_auth', 'auth');
	}

	/**
	 * Loads the navigation model
	 */
	function navigation()
	{
		$this->model(FASTCMS.'/model_navigation', 'navigation');
	}

	/**
	 * Loads the requests model
	 */
	function requests()
	{
		$this->model(FASTCMS.'/model_requests', 'requests');
	}
	...
	function users()
	{
		$this->model(FASTCMS.'/model_users', 'users');
	}
	...
	function pages()
	{
		$this->model(FASTCMS.'/model_pages', 'pages');
	}
	...
	function events()
	{
		$this->model(FASTCMS.'/model_events', 'events');
	}
	...
	function triggers()
	{
		$this->model(FASTCMS.'/model_triggers', 'triggers');
	}
	...
	{
		$this->model(FASTCMS.'/model_hierarchies', 'hierarchies');
	}
	...
	function settings()
	{
		$this->model(FASTCMS.'/model_settings', 'settings');
	}
	...
	function module($module_name, $params = array())
	{
		...
		{
			require_once(FASTCMSLIB . 'module.php');
		}
	...
	{
		require_once(APPPATH . '/libraries/' . FASTCMS . '/adapter.php');
		$this->library(FASTCMS.'/adapters/adapter_'.$name, NULL, $obj_name);
	}
