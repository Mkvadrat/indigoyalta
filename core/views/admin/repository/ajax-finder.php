...
?><div class="block withsidebar no_margin">
	...
	<div class="block_content">
		...
		<div class="sidebar">
	 	...
	 	<div class="sidebar_content whitebg" id="sb-repository">
			...
			<table cellpadding="0" cellspacing="0" width="100%" class="sortable repository">
				...
				<?php foreach ($repository_files as $file) { ?>
					<tr data-path="<?php echo $file->path; ?>">
						...
						<td class="delete"><img align="absmiddle" src="<?php echo site_url(THEMESPATH.'admin/widgets/icns/delete.png'); ?>" /> <a href="#" onclick="return fastcms.remove.document(this, '<?php echo $file->id_document; ?>');"><?php echo _('Delete file'); ?></a></td>
					</tr>
				<?php } ?>
				</tbody>
			</table>
		...
		foreach ($documents as $image)
		{
			$src = $image->thumb_path ? $image->thumb_path : $image->path;
						echo '<tr><td><img src="'. attach_url($src) . '" alt="" border="0" /></td>'
							...
							.'<td class="delete"><img align="absmiddle" src="'.site_url(THEMESPATH . 'admin/widgets/icns/delete.png').'" /> <a href="#" onclick="return fastcms.delete.document(this, '.$image->id_document.');">'._('Delete image').'</a></td>'
							.'</tr>';
...
function getPresetPath(el) {
	...
	_tr.find('.choose').unbind('click').click(function() { finder_choose(fastcms.preset_url(url, preset, true)); });
}
