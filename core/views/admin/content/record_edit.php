...
<div class="units-container tabbed-items">
	...
	<?php if ($first_lap == true){ $first_lap = false; if ($tipo['tree'])	{	if ($record->id && isset($page_url)) {
	$page_lang = $this->config->item('prepend_uri_language') ? $record->get('lang') : FALSE;
	$url = site_url($page_url, $page_lang); ?>
  <div class="current-location">
    <div class="units-row-end">
      <div class="unit-20">Текущий адрес страницы</div>
      <div class="unit-80"><a target="_blank" href="<?php echo $url; ?>"><?php echo $url; ?></a></div>
    </div>
  </div>
	<?php } } }
	...
	foreach ($fieldset['fields'] as $field_name)
	{
		...
		$reqpin = '<span>'.$field['description'].'</span>';
		$fieldcontainerclass = '';
		
		if ($field['mandatory'] && in_array($field['type'], array('text', 'number', 'date', 'radio', 'datetime')))
		{
			$fieldcontainerclass = ' mandatory-container';
			$reqpin = '<span class="bold">'.$field['description'].'</span><span class="req big">*</span>';
		...
		echo '<div class="fields-container field-'.$field_name.$hiddenclass.$fieldcontainerclass.'">';
		$label = '<div class="unit-20">'.$reqpin.'</div><div class="unit-80">';
		$label2 = '<div class="unit-20">Тест</div><div class="unit-80">';
		...
		switch ($field['type'])
		{
			...
			case 'visualbuilder':
					include(PARTSPATH.'fields/visualbuilder.php');
				break;
			...
			case 'thumbs':
					include(PARTSPATH.'fields/thumbs.php');
				break;

			case 'images':
					include(PARTSPATH.'fields/images_table.php');
				break;
			...
			case 'hierarchy':
					include(PARTSPATH.'fields/hierarchy.php');
				break;

			case 'textarea_short':
					include(PARTSPATH.'fields/textarea_short.php');
				break;

			case 'adress':
					include(PARTSPATH.'fields/adress.php');
				break;				
		}

		echo '</div>'; // fields-container end
...
<?php } ?>
	...
	<?php
		if ($tipo['has_categories']) {
			include(PARTSPATH.'tabs/categories.php');
		}
		if ($tipo['has_hierarchies']) {
			include(PARTSPATH.'tabs/hierarchies.php');
		}
		if (isset($tipo['relations'])) {
			include(PARTSPATH.'tabs/relations.php');
		}
	?>
	</div>
