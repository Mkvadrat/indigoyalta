<?php
	...
	$sharer_links = settings('yandex_share_block_links_cat');

?>
<div class="estate-list-wrapper">
	<div class="page-title"><h1><?php echo ($page_h1)? $page_h1:$page_title; ?></h1></div>

	<!--<?php if(preg_match( "#kvartiry-v-yalte-odnokomnatnye#", $_SERVER['REQUEST_URI'])){ ?>
	<p>"Мы вас подробно проконсультируем, подскажем как лучше <strong>купить квартиру в Ялте или ПГТ.</strong> Чтобы приобрести 1 комнатную из всех квартир в г. Ялта недорого лучше начинать поиски в зимнее время или в конце лета,
	как бы странно это не звучало. Наша многолетняя практика работы показала, что именно в эти периоды встречаются наиболее интересные предложения по приемлемой цене в рублях.</p>
    <p>Высококвалифицированные сотрудники агентства INDIGO, г. Ялта, помогут вам купить лучшую среди множества квартир самого города и ПГТ, например, Гурзуф, о уровне мастерства свидетельствуют положительные отзывы."</p>
	<?php } ?>-->
		...
		<ul class="estate-list">
			...
			<?php
				...
				foreach (records() as $record) {
                    ...
                    $descr_plaintext = strip_tags(html_entity_decode($record->get('short_content'), ENT_QUOTES, 'UTF-8'));

					if(empty($descr_plaintext)){
					...
					<div class="name">
						<div class="title-list"><span>№</span><span class="item-id"><?php echo $record->get('id_record'); ?></span><a href="<?php echo semantic_url($record); ?>"><?php echo $record->get('title'); ?></a></div>
						<?php if(!empty($sharer_links)){ ?>
								<div class="social_links">
									<div class="ya-share2" data-services="<?php echo $sharer_links; ?>" data-url="<?php echo semantic_url($record); ?>" data-size="s" data-counter=""></div>
								</div>
							<?php } ?>

					</div>
					<div class="thumbnail"><a href="<?php echo semantic_url($record); ?>"><?php if($specialrow){ ?><span class='img_special'></span><?php } ?><?php if($stateNew){ ?><span class="img_state_new"></span><?php } ?><img src="/assets/images/fotik.png" width="35" height="26" alt="Подробнее" class="fotik"><img src="<?php echo $thumbnailsrs;  ?>"  alt="<?php if(!empty($alt)){ echo $alt; }else{ echo htmlspecialchars(strip_tags($record->get('title'))); } ?>" ></a></div>
					<div class="information">
					    <div class="description-estate"><?php echo $descr_plaintext; ?><a href="<?php echo semantic_url($record); ?>">Подробнее</a></div>
					        <div class="adress-estate"><?php if(!empty($adress)){ ?>Адрес: <?php echo $adress;?><?php } ?></div>
					</div>
