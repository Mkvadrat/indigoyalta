...
Class requestform_Module extends Fastcms_Module
{
	/**
	 ...
	 */
	protected $from = 'webmaster@indigo-yalta.com';
			...
			switch ($this->action)
			{
				case 'email':
					...
					$request->set('author', $B->input->post('firstname'))
							...
							->set('objectid', $B->input->post('objectid'))
					...
					$parsed_msg = $B->parser->parse_string($msg, array(
						...
						'type'	=> $estatetypes[$B->input->post('type')],
						'objectid'	=> $B->input->post('objectid'),
